...
async function startHaruka() {
	...
	if (!sock.authState.creds.registered) {
		const phoneNumber = await question('\n\n\nKetik nomor kamu, contoh input nomor yang benar: 6283119115977\n');
			...
			for (let user of callData) {
				if (!user.isGroup && user.status === "offer") {
					try {
						...
						let warningMessage = `⚠️ *Ups, Kak! Mora gak bisa menerima panggilan ${callType}.*\n\n😔 Maaf banget, @${user.from.split('@')[0]}, panggilan seperti ini dapat membuat jaringan bot terganggu.`;
						if (user.from === `${ownerNumber}@s.whatsapp.net`) {
							await sock.rejectCall(user.id, user.from);
							await sock.sendMessage(user.from, { 
								text: warningMessage, 
								mentions: [user.from] 
							});
							console.log(`🚫 Panggilan dari ${user.from} ditolak tanpa diblokir.`);
						} else {
							await sock.rejectCall(user.id, user.from);
							await sock.sendMessage(user.from, { 
